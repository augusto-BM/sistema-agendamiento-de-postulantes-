...
// Función para cargar usuarios inactivos o buscar según query
async function cargarUsuariosInactivos(
  ...
  try {
    ...
    let busquedaRealizadaUsuariosDesactivos = $(
      "#busquedaRealizadaUsuariosDesactivos"
    );

    if (Array.isArray(response.users) && response.users.length > 0) {
      let counter = (pageInactivos - 1) * limitinactivos + 1;

      response.users.forEach(function (usuario) {
        tableBody.append(generarFilaUsuarioInactivos(usuario, counter));
        counter++;
      });

      totalRecordsInactivos = response.total;
      resultadoCountDesactivos.text(
        query
          ? `Se encontró ${totalRecordsInactivos} resultados de "${query}"`
          : `Se encontró ${totalRecordsInactivos} resultados`
      );
      busquedaRealizadaUsuariosDesactivos.text(
...
// Función para manejar la búsqueda y carga de usuarios inactivos
...
// Función para manejar la búsqueda de usuarios inactivos
...
$(document).ready(function () {
  cargarUsuariosInactivos(
    sedeSeleccionadaInactivos,
    pageInactivos,
    limitinactivos
  );
  // Eventos de búsqueda y filtros
  $("#buscarSearchDataInactivos").on("input", handleSearchInputInactivos);
  $("#btnBuscarSearchDataInactivos").on(
    "click",
    handleSearchButtonClickInactivos
  );
